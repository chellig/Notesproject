          ...
          <div class="row">
                  ...
                  <div class="offcanvas-body p-3 p-xl-0">
                    <div class="bg-dark border rounded-3 pb-0 p-3 w-100">
                      ...
                      <div class="list-group list-group-dark list-group-borderless">
                        <a class="list-group-item" href="/addenseignant">
                          <i class="bi bi-card-checklist fa-fw me-2"></i>Inscription
                        </a>
            ...
            <div class="col-xl-9">
              ...
              <div class="row mb-4">
                ...
                <div class="">
                  <h1 class="h3 mb-2 mb-sm-0">Liste des enseignants</h1>
                  <table class="table table-striped">
                    ...
                    <tbody  class="card-header p-4 border-bottom">
                      <tr v-for="enseignant in enseignants" :key="enseignant.id_enseignant">                                             
                        <td>{{ enseignant.nom }}</td>
                        <td>{{ enseignant.prenom }}</td>                      
                        <td>{{ enseignant.email }}</td>
                        <td>{{ enseignant.adresse }}</td>                      
                        <td>{{ enseignant.télephone }}</td>
                        <td>
                          <router-link
                            :to="{ name: 'editenseignant', params: { id_enseignant: enseignant.id_enseignant } }"
                            class="btn btn-outline-warning mx-2">
                            <i class="fa-solid fa-pen-to-square"></i> Modifier
                          </router-link>
                          <button @click="deleteenseignant(enseignant.id_enseignant)" class="btn btn-danger"><i class="fa-solid fa-trash-can"></i>Supprimer</button>
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
onMounted(() => {
  ...
  if (token) {
    ...
    const deleteenseignant = async (id_enseignant) => {
      if (window.confirm("Voulez-vous supprimer l'énseignant?")) {
        try {
          await axios.delete(`http://localhost:8000/api/enseignants/${id_enseignant}`);
